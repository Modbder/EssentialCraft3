...
public class TileMRUCoil extends TileMRUGeneric{
	...
	public boolean isStructureCorrect()
	{
		return isStructureCorrect && this.getMRU() > 1000;
	}
	...
	public void updateEntity()
	{
		...
		if(this.isStructureCorrect)
		{
			List<EntityLivingBase> entities = this.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, AxisAlignedBB.getBoundingBox(xCoord, yCoord, zCoord, xCoord+1, yCoord+1, zCoord+1).expand(rad, rad, rad));
			if(entities != null && !entities.isEmpty() && this.monsterLightning == null)
			{
				Ford:for(EntityLivingBase b : entities)
				{
					...
					{
						if(!((EntityPlayer)b).capabilities.isCreativeMode)
						{
	...
	{
		if(this.getMRU() > 1000 && !b.isDead && b.hurtTime <= 0 && b.hurtResistantTime <= 0)
		{
			b.attackEntityFrom(DamageSource.magic, 18);
