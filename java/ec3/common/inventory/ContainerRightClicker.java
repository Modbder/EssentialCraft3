...
public class ContainerRightClicker extends Container{
    ...
    public ContainerRightClicker(InventoryPlayer par1InventoryPlayer, TileEntity par2)
    {
        ...
        this.addSlotToContainer(new SlotBoundEssence(inv, 0, 26, 41));
        
        if(par2.getBlockMetadata() == 0 || par2.getBlockMetadata() == 1)
        {
	        this.addSlotToContainer(new Slot(inv, 1, 80, 24){
        ...
        }else
        {
        	for(int i = 0; i < 9; ++i)
        	{
    	        this.addSlotToContainer(new Slot(inv, 1+i, 62 + (i%3*18), 6 + (i/3*18)){
        ...
        this.addSlotToContainer(new Slot(inv, 10, 156, 4){
     	   ...
     	   @SideOnly(Side.CLIENT)
	   	    public IIcon getBackgroundIconIndex()
	   	    {
	   	        return Items.dye.getIconFromDamage(8);
	   	    }
