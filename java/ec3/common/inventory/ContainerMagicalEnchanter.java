...
public class ContainerMagicalEnchanter extends Container{
	
    private IInventory inv;
    public ContainerMagicalEnchanter(InventoryPlayer par1InventoryPlayer, TileEntity par2)
    {
        this.inv = (IInventory) par2;
        this.addSlotToContainer(new SlotBoundEssence(inv, 0, 155, 23));
        int i;
        this.addSlotToContainer(new Slot((IInventory) par2, 1, 26, 23));
        this.addSlotToContainer(new Slot((IInventory) par2, 2, 94, 23));
        for (i = 0; i < 3; ++i)
        {
            for (int j = 0; j < 9; ++j)
            {
