...
public class ContainerMagicalAssembler extends Container{
	
    private IInventory inv;
    public ContainerMagicalAssembler(InventoryPlayer par1InventoryPlayer, TileEntity par2)
    {
        this.inv = (IInventory) par2;
        this.addSlotToContainer(new SlotBoundEssence(inv, 0, 153, 5));
        this.addSlotToContainer(new Slot(inv, 1, 153, 41));
        for(int h = 2; h < 18; ++h)
        {
        	this.addSlotToContainer(new Slot(inv, h, 1000000, 1000000));
        }
        int i;
